		...
		if($this->uri->segment('3') == 'order_details'){
			...
			foreach($datas as $data_detail){
				...
				if(empty($zoneby) || in_array("1", $zoneby)){
					...
					$account_ref = ''; $invoice_to_practice = 0;
					if($data_detail['lab_id'] > 0){
						$userimData = array("user_id" => $data_detail['lab_id'], "column_name" => "'invoice_to_practice_immu'");
					...
					if ($total_allergen <= 4) {
						...
						$data["Vialnumber"] = '';
						$data["Order_is_to_be_sent_to"] = $order_send_to;
