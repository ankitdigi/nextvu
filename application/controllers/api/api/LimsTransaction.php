...
class LimsTransaction extends LIMS_API_Controller {
	...
	public function index_get($id = 0){
		...
		if(!empty($id)){
			...
			$this->db->where('ci_orders.is_draft', '0');
			$this->db->where('ci_orders.send_lims_sample', '1');
			$this->db->where('ci_orders.send_lims_transaction', '1');
			$datas = $this->db->get()->result_array();
		}
	...
	public function index_post($order_number){
		if($this->uri->segment('3') == 'authorised'){
			...
			$orderNumber = $order_number;
			$input = $this->input->post('transactionStatus');
			if(!isset($input)){
				$data = file_get_contents('php://input');
				$data = json_decode($data,true);
			...
			if($res->num_rows() == 0 ){
				...
				if($input == 1){
				$updtData['send_lims_transaction'] = 1;
				}else{
				$updtData['send_lims_transaction'] = 0;
				}
				$this->db->update('ci_orders', $updtData, array('id'=>$orderID));
