		...
		if($this->uri->segment('3') == 'order_details'){
			...
			if((isset($data['List_of_allergens']) && $data['List_of_allergens'] != "") && (isset($data['Sap_ID']) && $data['Sap_ID'] != "") && (isset($data['Customer_post_code']) && $data['Customer_post_code'] != "") && (isset($data['Order_date']) && $data['Order_date'] != "") && (isset($data['Species']) && $data['Species'] != "") && (isset($data['Patient_name']) && $data['Patient_name'] != "") && (isset($data['Owner_name']) && $data['Owner_name'] != "")){
				...
				if($total_allergen > 0){
					...
					$data_details["message"] = 'Order has been placed successfully.';
					$data_details["Nextmune_Order_Number"] = $final_order_number;
				}else{
					$data_details["message"] = 'Error in List of Allergens';
					$data_details["Nextmune_Order_Number"] = '';
				}
			}else{
				if($data['List_of_allergens'] != ""){
					$data_details["message"] = 'Error in List of Allergens';
				}elseif($data['Sap_ID'] != ""){
					$data_details["message"] = 'Error in Sap_ID';
				}elseif($data['Customer_post_code'] != ""){
					$data_details["message"] = 'Error in Customer_post_code';
				}elseif($data['Order_date'] != ""){
					$data_details["message"] = 'Error in Order_date';
				}elseif($data['Species'] != ""){
					$data_details["message"] = 'Error in Species';
				}elseif($data['Patient_name'] != ""){
					$data_details["message"] = 'Error in Patient_name';
				}elseif($data['Owner_name'] != ""){
					$data_details["message"] = 'Error in Owner_name';
				}
				$data_details["Nextmune_Order_Number"] = '';
			}
			$this->response($data_details, IDEXX_API_Controller::HTTP_OK);
		}
	}
