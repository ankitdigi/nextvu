...
class Lims extends LIMS_API_Controller {
	...
	public function index_get($id = 0){
		if($this->uri->segment('3') == 'lims_result'){
			$curl = curl_init();
			curl_setopt_array($curl, array(
				CURLOPT_URL => 'http://185.151.29.200:7070/Sample',
			...
			curl_close($curl);
			if(isset($error_msg)) {
				$this->response($error_msg, LIMS_API_Controller::HTTP_OK);
			}else{
				...
				if(!empty($results)){
					...
					foreach($results as $resultL){
						...
						$this->db->where('lab_order_number', $resultL['limsId']);
						...
						if($orderID != ""){
							...
							$ordersData['updated_by'] = $this->user_id;
					...
					if($count > 0){
						$this->response('LIMS Result data added successfully.', LIMS_API_Controller::HTTP_OK);
					}else{
						$this->response('LIMS Result data are empty.', LIMS_API_Controller::HTTP_OK);
					}
				}else{
					$this->response('LIMS Result data are empty.', LIMS_API_Controller::HTTP_OK);
				}
			...
			if(!empty($id)){
				...
				$this->db->where('ci_orders.send_lims', '0');
			...
			foreach($datas as $data_detail){
				...
				if(!empty($data_detail['product_code_selection'])){
					$this->db->select('name,lims_name,lims_code');
