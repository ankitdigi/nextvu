...
class Skintest extends REST_Controller {
	...
	public function index_get($id = 0){
		if($this->uri->segment('3') == 'order_details'){
			...
			$this->db->where('ci_orders.order_type', '3');
			...
			foreach($datas as $data_detail){
				...
				$account_ref = '';
				if($data_detail['lab_id'] > 0){
					/* if($data_detail['order_can_send_to'] == '1' && ($data_detail['lab_id'] == '108' || $data_detail['lab_id'] == '13788')){ */
					if($data_detail['order_can_send_to'] == '1'){
				...
				$sqls = "SELECT u.country, c.name FROM `ci_users` as u LEFT JOIN `ci_staff_countries` as c ON u.country = c.id WHERE u.id = '".$usrID."'";
				$responced = $this->db->query($sqls);
				$usrquery = $responced->row();
				$country = !empty($usrquery->name)?$usrquery->name:'UK';
				...
				if(!empty($alphaArr)){
					foreach($alphaArr as $akey=>$avalue){
						...
						$data["Invoice_To"] = "120220";
						if($country == "UK" || $account_ref == '113898'){
							$data["Freight_Cost"] = "0.00";
						}else{
							$data["Freight_Cost"] = $freightCost;
						}
						$data["Itemcode"] = 'VI'.$avalue;
						/* $data["Vialnumber"] = ''; */
				...
				if(!empty($numericArr)){
					foreach($numericArr as $nkey=>$nvalue){
						$data["UniqueIdentifier"] = $data_detail['order_number'];
						$data["Reference_Number"] = ($data_detail['reference_number']==NULL) ? "" : $data_detail['reference_number'];
						$data["Orderby"] = $account_ref;
						$data["Country"] = $country;
						...
						$data["Itemcode"] = 'VI'.$nvalue;
						/* $data["Vialnumber"] = ''; */
						$data["Order is to be sent to"] = $order_send_to;
						$data_details[] = $data;
					}
				}
				/* if ($total_allergen > 24) {
					$quotient = ($total_allergen/24);
					...
					for ($x = 1; $x <= $totalVials; $x++) {
						...
						$data["Itemcode"] = 'VI'.implode("",$allergeName[$x]);
						$data["Vialnumber"] = $x;
						$data["Order is to be sent to"] = $order_send_to;
						$data_details[] = $data;
					}
					...
					sort($alphaArr);
					sort($numericArr, SORT_NUMERIC);
					$allergeName = 'VI'.implode("",$alphaArr).implode("",$numericArr);
					$data["UniqueIdentifier"] = $data_detail['order_number'];
					$data["Reference_Number"] = ($data_detail['reference_number']==NULL) ? "" : $data_detail['reference_number'];
					$data["Orderby"] = $account_ref;
					$data["Country"] = $country;
