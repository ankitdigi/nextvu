	...
	function sub_addEdit($id= ''){
		...
		if ($this->input->post('name')) {
            ...
            if ($this->form_validation->run() == FALSE){
				...
				$sub_allergenData['name_danish'] = $this->input->post('name_danish');
				$sub_allergenData['name_french'] = $this->input->post('name_french');
				$sub_allergenData['name_german'] = $this->input->post('name_german');
				$sub_allergenData['name_italian'] = $this->input->post('name_italian');
				$sub_allergenData['name_dutch'] = $this->input->post('name_dutch');
				$sub_allergenData['name_norwegian'] = $this->input->post('name_norwegian');
				$sub_allergenData['name_spanish'] = $this->input->post('name_spanish');
				$sub_allergenData['name_swedish'] = $this->input->post('name_swedish');
				$sub_allergenData['pax_name'] = $this->input->post('pax_name');
				$sub_allergenData['pax_name_danish'] = $this->input->post('pax_name_danish');
				$sub_allergenData['pax_name_french'] = $this->input->post('pax_name_french');
				$sub_allergenData['pax_name_german'] = $this->input->post('pax_name_german');
				$sub_allergenData['pax_name_italian'] = $this->input->post('pax_name_italian');
				$sub_allergenData['pax_name_dutch'] = $this->input->post('pax_name_dutch');
				$sub_allergenData['pax_name_norwegian'] = $this->input->post('pax_name_norwegian');
				$sub_allergenData['pax_name_spanish'] = $this->input->post('pax_name_spanish');
				$sub_allergenData['pax_name_swedish'] = $this->input->post('pax_name_swedish');
							...
							if(!empty($raptorCodeArr)){
								foreach($raptorCodeArr as $key=>$value){
									if($value != "" || ($value == "" && $emAllergen[$key] == '1')){
										...
										/* $rHeader = !empty($raptorHeader[$key][1])?preg_replace("/'/i", "\'", $raptorHeader[$key][1]):array();
										$rdanHeader = !empty($raptorHeader[$key][2])?preg_replace("/'/i", "\'", $raptorHeader[$key][2]):array();
										$rfreHeader = !empty($raptorHeader[$key][3])?preg_replace("/'/i", "\'", $raptorHeader[$key][3]):array();
										$rgerHeader = !empty($raptorHeader[$key][4])?preg_replace("/'/i", "\'", $raptorHeader[$key][4]):array();
										$ritaHeader = !empty($raptorHeader[$key][5])?preg_replace("/'/i", "\'", $raptorHeader[$key][5]):array();
										$rdutHeader = !empty($raptorHeader[$key][6])?preg_replace("/'/i", "\'", $raptorHeader[$key][6]):array();
										$rnorHeader = !empty($raptorHeader[$key][7])?preg_replace("/'/i", "\'", $raptorHeader[$key][7]):array();
										$rspaHeader = !empty($raptorHeader[$key][8])?preg_replace("/'/i", "\'", $raptorHeader[$key][8]):array();
										$rsweHeader = !empty($raptorHeader[$key][9])?preg_replace("/'/i", "\'", $raptorHeader[$key][9]):array(); */
										$rHeader = !empty($raptorHeader[$key][1])?$raptorHeader[$key][1]:array();
										$rdanHeader = !empty($raptorHeader[$key][2])?$raptorHeader[$key][2]:array();
										$rfreHeader = !empty($raptorHeader[$key][3])?$raptorHeader[$key][3]:array();
										$rgerHeader = !empty($raptorHeader[$key][4])?$raptorHeader[$key][4]:array();
										$ritaHeader = !empty($raptorHeader[$key][5])?$raptorHeader[$key][5]:array();
										$rdutHeader = !empty($raptorHeader[$key][6])?$raptorHeader[$key][6]:array();
										$rnorHeader = !empty($raptorHeader[$key][7])?$raptorHeader[$key][7]:array();
										$rspaHeader = !empty($raptorHeader[$key][8])?$raptorHeader[$key][8]:array();
										$rsweHeader = !empty($raptorHeader[$key][9])?$raptorHeader[$key][9]:array();
										$codeInfo = array('allergens_id'=>$id, 'raptor_code'=>$value, 'raptor_function'=>$rFunction, 'em_allergen'=>$rAllergen, 'raptor_header'=>json_encode($rHeader), 'raptor_header_danish'=>json_encode($rdanHeader), 'raptor_header_french'=>json_encode($rfreHeader), 'raptor_header_german'=>json_encode($rgerHeader), 'raptor_header_italian'=>json_encode($ritaHeader), 'raptor_header_dutch'=>json_encode($rdutHeader), 'raptor_header_norwegian'=>json_encode($rnorHeader), 'raptor_header_spanish'=>json_encode($rspaHeader), 'raptor_header_swedish'=>json_encode($rsweHeader));
