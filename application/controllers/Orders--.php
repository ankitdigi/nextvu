...
class Orders extends CI_Controller{
	...
	function allergens($id = ''){
		...
		if($data['order_type'] == '2') {
			if($data['serum_type'] == '1'){
				...
				$this->db->from('ci_price');
				$this->db->where('id', $data['product_code_selection']);
				...
				$allergens_group = $this->AllergensModel->get_allergens_dropdown($subOrderTypeArr);
	...
	function print_form($id = ''){
		...
		$html = $this->load->view('orders/print_form_template', $this->_data, true);
		$html = trim($html);
		$dompdf->loadHtml($html);
		$dompdf->setPaper('A4', 'Portrait');
		$dompdf->render();
		$pdf = $dompdf->output();
		$file = FCPATH . SERUM_REQUEST_PDF_PATH . "serum_request_form_" . $data['order_number'] . ".pdf";
		file_put_contents($file, $pdf);
		//redirect('orders');
	...
	public function send_serum_mail($id = '', $is_confirmed = 0){
											...
											<tr>
												<td align="left" style="font-family: Open Sans, Helvetica, Arial, sans-serif; font-size: 16px; font-weight: 400; line-height:20px; padding-top: 10px;">
													<p style="font-size: 16px; font-weight: 400; line-height: 20px; color: #777777;">
														The test results for your patient '.$data['pet_name'].' '.$data['po_last'].' have been completed. Please view results by visiting our online portal NextView or by downloading the result booklet.
													</p>
												</td>
											</tr>
											<tr>
		...
		$dompdf->render();
		$pdf = $dompdf->output();
		$file1 = FCPATH . SERUM_REQUEST_PDF_PATH . "serum_test_result_". $order_number .".pdf";
		file_put_contents($file1, $pdf);
		...
		//$this->email->attach($this->createTreatmentPDF($this->_data));
	...
	function getSerumResultdoc($orderId){
		...
		if($orderId > 0){
			...
			$respnedn = $this->db->get()->row();
			if($data['serum_type'] == 1){
				$redirectlink = 'orders/interpretation/';
				...
				$dochtml .= '<p><b>Synlab Lab Ref:</b> '.$data['lab_order_number'].'</p>';
				$dochtml .= '<p><b>Nextmune Ref:</b> '.$raptorData->barcode.'</p>';
				$dochtml .= '<p></p>';
				if(!empty($respnedn)){
					if((preg_match('/\bEnvironmental\b/', $respnedn->name)) && (!preg_match('/\bFood\b/', $respnedn->name))){
						$fileName = 'PAX_Environmental_Serum_Result_'.$data['order_number'].'';
						$dochtml .= '<p><b>Environmental Panel</b></p>';
					}elseif((!preg_match('/\bEnvironmental\b/', $respnedn->name)) && (preg_match('/\bFood\b/', $respnedn->name))){
						$fileName = 'PAX_Food_Serum_Result_'.$data['order_number'].'';
						$dochtml .= '<p><b>Food Panel</b></p>';
					}elseif((preg_match('/\bEnvironmental\b/', $respnedn->name)) && (preg_match('/\bFood\b/', $respnedn->name))){
						$fileName = 'PAX_Environmental+Food_Serum_Result_'.$data['order_number'].'';
						$dochtml .= '<p><b>Environmental + Food Panel</b></p>';
					}
					$getAllergenParent = $this->AllergensModel->getAllergenParentbyName($data['allergens']);
					foreach ($getAllergenParent as $apkey => $apvalue){
						$dochtml .= '<p></p>';
						$dochtml .= '<p><b>'.$apvalue['pax_name'].'</b></p>';
						$dochtml .= '<table><tbody>';
						...
						foreach ($subAllergens as $skey => $svalue) {
							...
							if(!empty($subVlu->raptor_code)){
								...
								if(!empty($raptrVlu)){
									$dochtml .= '<tr>
										<td width="250">'.$svalue['pax_name'].'</td>
										<td width="50">'.round($raptrVlu->result_value).'</td>
										<td width="100">POSITIVE</td>
									</tr>
				...
				if(!empty($respnedn)){
					...
					$dochtml .= '<p><b>Species:</b> '.$data['species_name'].'</p>';
					$dochtml .= '<p><b>Synlab Lab Ref:</b> '.$data['lab_order_number'].'</p>';
					$dochtml .= '<p><b>Nextmune Ref:</b> </p>';
					$dochtml .= '<p></p>';
					if((preg_match('/\bEnvironmental\b/', $respnedn->name)) && (!preg_match('/\bFood\b/', $respnedn->name))){
						$fileName = 'NextLab_Environmental_Serum_Result_'.$data['order_number'].'';
						...
						if(!empty($serumResultsenv)){
							...
							foreach($serumResultsenv as $row1){
								$dochtml .= '<tr>
									<td width="250">'.$row1->name.'</td>
									<td width="50">'.$row1->result.'</td>
									<td width="100">'.$row1->outcome.'</td>
								</tr>
								<tr><td colspan="3"></td></tr>';
							}
							$dochtml .= '</tbody></table>';
						...
						$fileName = 'NextLab_Food_Serum_Result_'.$data['order_number'].'';
						...
						if(!empty($serumResultsfod)){
							...
							foreach($serumResultsfod as $rowf){
								...
								$dochtml .= '<tr>
									<td width="250">'.$rsultFIge->name.'</td>
									<td width="50">'.$rsultFIge->result.'</td>
									<td width="100">'.$rsultFIge->outcome.'</td>
								</tr>
								<tr><td colspan="3"></td></tr>';
								$rsultFIgg = $this->OrdersModel->getSerumTestResultFoodIGG($rowf->name,$sresultID,$stypeID);
								$dochtml .= '<tr>
									<td width="250">'.$rsultFIgg->name.'</td>
									<td width="50">'.$rsultFIgg->result.'</td>
									<td width="100">'.$rsultFIgg->outcome.'</td>
								</tr>
								<tr><td colspan="3"></td></tr>';
							}
							$dochtml .= '</tbody></table>';
						...
						$fileName = 'NextLab_Environmental+Food_Serum_Result_'.$data['order_number'].'';
						$dochtml .= '<p><b>Environmental Panel</b></p>';
						$dochtml .= '<p></p>';
							...
							foreach($sub2Allergens as $s2value){
								...
								if(!empty($serumResults)){
									$dochtml .= '<tr>
										...
										<td width="100">'.$serumResults->outcome.'</td>
									</tr>
									<tr><td colspan="3"></td></tr>';
								}
							}
							...
							foreach($subfAllergens as $sfvalue){
								...
								if(!empty($serumfResults)){
									$dochtml .= '<tr>
										...
										<td width="100">'.$serumfResults->outcome.'</td>
									</tr>
									<tr><td colspan="3"></td></tr>';
								}
							}
	...
	function sendPaxResultNotificationforPetOwner($orderId){
		if($orderId > 0){
			...
			$pdf = $dompdf->output();
			$file = FCPATH . SERUM_REQUEST_PDF_PATH . "serum_result_" . $data['order_number'] . ".pdf";
			file_put_contents($file, $pdf);
