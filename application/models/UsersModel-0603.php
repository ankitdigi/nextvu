...
class UsersModel extends CI_model{
    ...
    public function getRecord($id="",$role_id="") {
      if($role_id==5){
        ...
        //$this->db->select("ci_user_details.*,GROUP_CONCAT(CONCAT(column_name, '=', IFNULL(column_field, ''))) AS column_field,ci_users.name,ci_users.email");
        //$this->db->select("ci_user_details.*,CONCAT( '{', GROUP_CONCAT( CONCAT( '', column_name, ':', column_field, '' ) ), '}' ) AS column_field,ci_users.name,ci_users.email");
	...
	function getRecordAll($role_id='',$user_id='',$user_role='',$column_name='',$user_country='') {
		...
		if($role_id=='2'){
			...
			$this->db->where('ci_user_details.column_name','address_3');
		}
		if($role_id=='5'){
